...
class CourseViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        elif self.action in ['update', 'partial_update', 'destroy', 'publish', 'unpublish'] + deck_actions:
    ...
    def _check_deck_permissions(self, deck_id: int, course: Course, user: UserContext) -> None:
    ...
    def decks(self, request, pk=None):
        ...
        if request.method.lower() == 'get':
            ...
            serializer = CourseDeckSerializer(queryset, many=True, context={'request': request})
    ...
    def decks_reorder(self, request, pk=None):
        ...
        if not isinstance(items, list):
            raise ValidationError({'detail': 'Expected a list of objects with id and order_index'})
        ...
        for item in items:
            ...
            if 'id' not in item or 'order_index' not in item:
                raise ValidationError({'detail': 'Each item must have id and order_index'})
            ids.append(item['id'])
        ...
        if len(course_decks) != len(set(ids)):
            raise ValidationError({'detail': 'Some course_deck ids do not belong to this course'})
        ...
        return Response(CourseDeckSerializer(ordered, many=True, context={'request': request}).data)
    ...
    def decks_batch(self, request, pk=None):
        ...
        existing_deck_ids = set(
            course.course_decks.filter(deck_id__in=deck_ids).values_list('deck_id', flat=True)
        )
        ...
        serializer = CourseDeckSerializer(created_decks, many=True, context={'request': request})
    ...
    @action(detail=True, methods=['get', 'patch', 'delete'], url_path='decks/(?P<deck_rel_id>\\d+)')
    def deck_detail(self, request, pk=None, deck_rel_id=None):
        ...
        if request.method.lower() == 'get':
            return Response(CourseDeckSerializer(coursedeck, context={'request': request}).data)
        ...
        # PATCH
        serializer = CourseDeckCreateUpdateSerializer(coursedeck, data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
