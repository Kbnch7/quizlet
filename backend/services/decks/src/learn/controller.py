...
def _session_to_response(learn_session) -> SLearnSessionResponse:
    return SLearnSessionResponse(
        ...
        ended_at=(
            learn_session.ended_at.isoformat()
            if learn_session.ended_at
            else None
        ),
    )
...
@router.post(
    "/deck/{deck_id}/sessions", response_model=SLearnSessionCreateResponse
)
...
@router.get(
    "/sessions/{session_id}/progress", response_model=SLearnProgressResponse
)
...
@router.post(
    "/sessions/{session_id}/finish", response_model=SLearnProgressResponse
)
