...
class TestCreateCategory:
    ...
    async def test_create_category_unauthorized(self, client: AsyncClient):
        ...
        assert "Not authenticated" in response.json()["detail"] or "Not authenticated" in response.json()["detail"]
    ...
    async def test_create_category_duplicate_slug(
        ...
        assert test_category.slug in data["error"] or data.get("value") == test_category.slug
    ...
    async def test_create_category_duplicate_name(
        ...
        assert test_category.name in data["error"] or data.get("value") == test_category.name
...
class TestUpdateCategory:
    ...
    async def test_update_category_duplicate_slug(
        ...
        assert "error" in data
        assert "slug" in data["error"] or data.get("field") == "slug"
        assert test_categories[0].slug in data["error"] or data.get("value") == test_categories[0].slug
    ...
    async def test_update_category_duplicate_name(
        ...
        assert "error" in data
        assert "name" in data["error"] or data.get("field") == "name"
        assert test_categories[0].name in data["error"] or data.get("value") == test_categories[0].name
