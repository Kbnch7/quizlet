...
class TestCreateCard:
    ...
    async def test_create_card_unauthorized(self, client: AsyncClient, test_deck: Deck):
        ...
        assert "Not authenticated" in response.json()["detail"] or "Unauthenticated" in response.json()["detail"]
    ...
    async def test_create_card_forbidden(
        ...
        response = await client.post(f"/deck/{test_deck.id}/cards", json=payload, headers=headers)
    ...
    async def test_create_card_with_images(
        ...
        assert "front_image_url" in data or data.get("front_image_url") is not None
        assert "back_image_url" in data or data.get("back_image_url") is not None
...
class TestUpdateCard:
    ...
    async def test_update_card_unauthorized(
        ...
        response = await client.patch(
            f"/deck/{test_deck.id}/cards/{test_card.id}", 
            json=payload
        )
        assert response.status_code == 401
        assert "Not authenticated" in response.json()["detail"] or "Unauthenticated" in response.json()["detail"]
...
class TestDeleteCard:
    ...
    async def test_delete_card_not_found(
        ...
        headers = get_auth_headers(mock_user)
        response = await client.delete(f"/deck/{test_deck.id}/cards/999", headers=headers)
        assert response.status_code == 404
        assert "Card not found" in response.json()["detail"]
    ...
    async def test_delete_card_wrong_deck(
        ...
        headers = get_auth_headers(mock_user)
        response = await client.delete(f"/deck/999/cards/{test_card.id}", headers=headers)
        assert response.status_code == 404
        assert "Card not found" in response.json()["detail"]
    ...
    async def test_delete_card_success(
        ...
        headers = get_auth_headers(mock_user)
        response = await client.delete(
            f"/deck/{test_deck.id}/cards/{card.id}", 
            headers=headers
        )
...
class TestBulkCards:
    ...
    async def test_bulk_cards_unauthorized(
        self, 
        client: AsyncClient, 
        test_deck: Deck
    ):
    ...
    async def test_bulk_cards_deck_not_found(
        ...
        headers = get_auth_headers(mock_user)
        payload = [{"front_text": "Q1", "back_text": "A1"}]
        response = await client.patch("/deck/decks/999/cards", json=payload, headers=headers)
        assert response.status_code == 404
        assert "Deck not found" in response.json()["detail"]
    ...
    async def test_bulk_cards_forbidden(
        ...
        payload = [{"front_text": "Q1", "back_text": "A1"}]
        response = await client.patch(
            f"/deck/decks/{test_deck.id}/cards", 
            json=payload, 
            headers=headers
        )
        assert response.status_code == 403
