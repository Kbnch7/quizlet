...
class TestCreateLearnSession:
    ...
    async def test_create_learn_session_success(
        ...
        response = await client.post(
            f"/learn/deck/{test_deck.id}/sessions", headers=headers
        )
    ...
    async def test_create_learn_session_unauthorized(
        self, client: AsyncClient, test_deck: Deck
    ):
    ...
    async def test_create_learn_session_deck_not_found(
        ...
        headers = get_auth_headers(mock_user)
        response = await client.post("/learn/deck/99999/sessions", headers=headers)
    ...
    async def test_create_learn_session_deck_no_cards(
        ...
        await db_session.execute(delete(Card).where(Card.deck_id == test_deck.id))
...
class TestGetLearnSession:
    ...
    async def test_get_learn_session_success(
        ...
        # Получаем сессию
        response = await client.get(f"/learn/sessions/{session_id}", headers=headers)
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_get_learn_session_not_found(
        ...
        headers = get_auth_headers(mock_user)
        response = await client.get("/learn/sessions/99999", headers=headers)
        assert response.status_code == 404
    ...
    async def test_get_learn_session_other_user(
        ...
        response = await client.get(
            f"/learn/sessions/{session_id}", headers=other_headers
        )
        assert response.status_code == 404
        assert "Learn session not found" in response.json()["detail"]
...
class TestGetNextCard:
    ...
    async def test_get_next_card_success(
        ...
        response = await client.get(
            f"/learn/sessions/{session_id}/next", headers=headers
        )
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_get_next_card_session_not_found(
        ...
        headers = get_auth_headers(mock_user)
        response = await client.get("/learn/sessions/99999/next", headers=headers)
        assert response.status_code == 404
        assert "Learn session not found" in response.json()["detail"]
    ...
    async def test_get_next_card_session_not_active(
        ...
        finish_response = await client.post(
            f"/learn/sessions/{session_id}/finish", headers=headers
        )
...
class TestSubmitAnswer:
    ...
    async def test_submit_answer_correct(
        ...
        payload = {"correct": True, "answer_time_seconds": 5}
        response = await client.post(
    ...
    async def test_submit_answer_incorrect(
        ...
        payload = {"correct": False, "answer_time_seconds": 10}
        response = await client.post(
            f"/learn/sessions/{session_id}/cards/{card_id}/answer",
            json=payload,
            headers=headers,
    ...
    async def test_submit_answer_unauthorized(
        ...
        payload = {"correct": True, "answer_time_seconds": 5}
        response = await client.post(
            f"/learn/sessions/{session_id}/cards/{card_id}/answer", json=payload
        )
        assert response.status_code == 401
        assert "Not authenticated" in response.json()["detail"]
    ...
    async def test_submit_answer_session_not_found(
        ...
        payload = {"correct": True, "answer_time_seconds": 5}
        response = await client.post(
            "/learn/sessions/99999/cards/1/answer", json=payload, headers=headers
        )
        assert response.status_code == 404
        assert "Learn session not found" in response.json()["detail"]
...
class TestGetSessionProgress:
    ...
    async def test_get_session_progress_success(
        ...
        # Получаем прогресс
        response = await client.get(
            f"/learn/sessions/{session_id}/progress", headers=headers
        )
        assert response.status_code == 200
        data = response.json()
        assert data["session_id"] == session_id
    ...
    async def test_get_session_progress_not_found(
        ...
        headers = get_auth_headers(mock_user)
        response = await client.get("/learn/sessions/99999/progress", headers=headers)
        assert response.status_code == 404
        assert "Learn session not found" in response.json()["detail"]
...
class TestFinishLearnSession:
    ...
    async def test_finish_learn_session_success(
        ...
        get_response = await client.get(
            f"/learn/sessions/{session_id}", headers=headers
        )
    ...
    async def test_finish_learn_session_not_found(
        ...
        headers = get_auth_headers(mock_user)
        response = await client.post("/learn/sessions/99999/finish", headers=headers)
        assert response.status_code == 404
        assert "Learn session not found" in response.json()["detail"]
