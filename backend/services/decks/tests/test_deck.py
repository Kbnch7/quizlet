...
from src.entities import Deck, Category
...
class TestGetDecks:
    ...
    async def test_get_decks_filter_by_category(
        ...
        headers = get_auth_headers(mock_user)
        response = await client.get(f"/deck/?category={test_categories[0].slug}", headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert len(data) >= 1
    ...
    async def test_get_decks_with_cursor(
        ...
        if cursor:
            response = await client.get(f"/deck/?limit=2&cursor={cursor}", headers=headers)
            assert response.status_code == 200
...
class TestGetDeck:
    ...
    async def test_get_deck_not_found(self, client: AsyncClient, mock_user: UserContext):
...
class TestUpdateDeck:
    ...
    async def test_update_deck_unauthorized(
        self, 
        client: AsyncClient, 
        test_deck: Deck
    ):
    ...
    async def test_update_deck_forbidden(
        ...
        payload = {"title": "Updated Title"}
        response = await client.patch(f"/deck/{test_deck.id}/", json=payload, headers=headers)
        assert response.status_code == 403
        assert "Forbidden" in response.json()["detail"]
...
class TestDeleteDeck:
    ...
    async def test_delete_deck_unauthorized(
        self, 
        client: AsyncClient, 
        test_deck: Deck
    ):
...
class TestDeckStats:
    ...
    async def test_deck_stats_unauthorized(
        self, 
        client: AsyncClient, 
        test_deck: Deck
    ):
